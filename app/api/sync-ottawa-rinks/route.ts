...
const OTTAWA_RINKS_COMPLETE = [
  ...
  // === PRIVATE/SEMI-PRIVATE FACILITIES (25个) ===
  // Sensplex (Official Senators Practice Facilities)
  ...
  try {
    ...
    for (const rink of OTTAWA_RINKS_COMPLETE) {
      try {
        ...
        const { data: existingRink } = await supabase
          .from('rinks')
