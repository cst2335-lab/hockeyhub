...
| 15 | Browser console check | DevTools Console on main happy-path pages | No unexpected blocking red errors on demo path | PASS | Demo path pages rendered and navigated. Initial Cursor IDE browser pass showed a React hydration mismatch on `/en/dashboard` (`DashboardLayout`); the diff only removed injected **`data-cursor-ref`** attrs (Cursor DOM tooling), not app props. **Follow-up 2026-07-26:** clean Playwright Chromium (`--disable-extensions`, no Cursor browser) logged into `/en/dashboard`, `data-cursor-ref` count **0**, **no hydration / mismatch console messages**. Treat the Cursor warning as **non-blocking browser-extension / Cursor DOM-injection noise**, not an app hydration defect. (Unrelated: Next Image LCP/`icon.svg` warnings; occasional network 400 — not claimed fixed here.) |
