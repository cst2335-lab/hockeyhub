...
- **已完成（V2 P2 · 2026-04-06）**：**XSS**（JSON-LD 递归字符串清洗；比赛与 legacy 列表对用户可见字段 `sanitizePlainText`）、**SEO JSON-LD**（预订详情 `Reservation` + `noindex`；俱乐部列表 metadata；既有页面经统一 `serializeJsonLd`）。
...
### 6.2 已完成（V2 P2 · 与 NEXT_PHASE_TASKS §二 一致）

- [x] **XSS 防护**：JSON-LD 与关键 UI 路径已按 `sanitizePlainText` / `serializeJsonLd` 收口（见 CHANGELOG §二十二）  
- [x] **SEO JSON-LD**：比赛、订场、站点、预订详情、俱乐部列表等已覆盖或补强  
